...
- **`v0.11.3` (tagged 2026-08-31 ~14:55 UTC) supersedes v0.11.2 as the upgrade target** — v0.11.2 was a **security patch** (request "some security fixes withheld"; recommends updating production promptly) and fixes the reasoning-model streaming stall (#29053) + thinking rendered into replies after tool calls (#29052), both relevant to the Qwen tutor workaround; **v0.11.3 is a strict superset** (verified `git merge-base --is-ancestor v0.11.2 v0.11.3` → exit 0, so it carries ALL of v0.11.2's security fixes) and **additionally fixes the upgrade-failure bug #29280** — a failed DB migration now aborts cleanly instead of half-booting and later reporting a missing `chat.timer_at` column (exactly the `d4c1a8e37b62` chat-timer migration in our 0.11.0→0.11.2 path), so v0.11.3 is the safer upgrade target. **Schema-verified 2026-09-01**: the v0.11.3 Alembic graph is identical to v0.11.2 — 58 revisions, zero merge points, single head `d4c1a8e37b62`, exactly the same 3 linear migrations on top of `f0bd01a18a3d` (zero migration-file diff v0.11.2→v0.11.3; no new migration in 0.11.3). Image `v0.11.3` pre-pulled locally. An upgrade therefore goes straight to `WEBUI_DOCKER_TAG=v0.11.3` — nothing extra to verify beyond the standing plan (backup 7/7 → staged `up -d --force-recreate` pilot+6 → health/version/alembic/preflight ×7 → rollback = PG restore; the recreate also repairs epita's torn log file).
