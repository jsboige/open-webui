...
for tenant in epf epita esg ece epf-genai pauwels; do
    ...
    create_or_update_model "$URL" "$TOKEN" "Qwen_think" \
        '{"id":"Qwen_think","name":"Qwen 3.5 Think (Général)","base_model_id":"Local.qwen3.5-35b-a3b","params":{"temperature":0.7,"presence_penalty":1.5,"top_p":0.95,"max_tokens":8192,"custom_params":{"top_k":20,"chat_template_kwargs":{"enable_thinking":true}}},"meta":{"profile_image_url":"/static/favicon.png","description":"Qwen3.5-35B thinking mode, sampling optimisé général (temp 0.7, pp 1.5). Calibré AWQ Q4.","capabilities":null}}'

    create_or_update_model "$URL" "$TOKEN" "Qwen_think-code" \
        '{"id":"Qwen_think-code","name":"Qwen 3.5 Think Code","base_model_id":"Local.qwen3.5-35b-a3b","params":{"temperature":0.6,"presence_penalty":0.0,"top_p":0.95,"max_tokens":16384,"custom_params":{"top_k":20,"chat_template_kwargs":{"enable_thinking":true}}},"meta":{"profile_image_url":"/static/favicon.png","description":"Qwen3.5-35B thinking mode optimisé code (temp 0.6, pas de pénalité). Preset officiel Qwen.","capabilities":null}}'

    create_or_update_model "$URL" "$TOKEN" "Qwen_think-reason" \
        '{"id":"Qwen_think-reason","name":"Qwen 3.5 Think Reason","base_model_id":"Local.qwen3.5-35b-a3b","params":{"temperature":1.0,"presence_penalty":1.5,"top_p":1.0,"max_tokens":16384,"custom_params":{"top_k":40,"chat_template_kwargs":{"enable_thinking":true}}},"meta":{"profile_image_url":"/static/favicon.png","description":"Qwen3.5-35B thinking mode raisonnement profond (temp 1.0, pp 1.5, top_k 40). Max diversité.","capabilities":null}}'

    create_or_update_model "$URL" "$TOKEN" "Qwen_instruct" \
        '{"id":"Qwen_instruct","name":"Qwen 3.5 Instruct (Chat rapide)","base_model_id":"Local.qwen3.5-35b-a3b","params":{"temperature":0.7,"presence_penalty":1.5,"top_p":0.8,"min_p":0.01,"max_tokens":4096,"custom_params":{"top_k":20,"repetition_penalty":1.1,"chat_template_kwargs":{"enable_thinking":false}}},"meta":{"profile_image_url":"/static/favicon.png","description":"Qwen3.5-35B mode instruct sans thinking. Réponses rapides, anti-bleed Q4 (rp 1.1).","capabilities":null}}'
